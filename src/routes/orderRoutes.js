...
router.get('/', async (req, res) => {
  try {
    const orders = await Order.find(); // Lấy tất cả orders từ MongoDB
    res.json(orders); // Trả dữ liệu về frontend
  } catch (error) {
...
router.get('/user', async (req, res) => {
  try {
    const orders = await Orderuser.find(); // Lấy tất cả orders từ MongoDB
    res.json(orders); // Trả dữ liệu về frontend
  } catch (error) {
    console.error('Error fetching orders:', error);
    res.status(500).json({ message: 'Server error while fetching orders' });
